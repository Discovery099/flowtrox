...
def load_instrument_data(symbol: str = "ES"):
    ...
    validate_data(df)
    train_df, test_df = split_train_test(df, inst["train_end_index"])
...
def _evaluate_on_test(test_df: pd.DataFrame, fitted: dict, params: dict) -> dict:
    ...
    signals = generate_signals(test_feat, params)
...
def run_single_backtest(
    ...
    evald = _evaluate_on_test(test_df, fitted, params)
    return {
        "symbol": symbol,
        "params": params,
...
def run_full_pipeline(
    ...
    evald = _evaluate_on_test(test_df, fitted, best_params)
